...
Monkey::Monkey(std::unique_ptr<IFood> food) : _food(std::move(food)) {
}
...
void Monkey::eatFavouriteFood(std::ostream &stream) {
    stream << "Monkey eats " << _food->name() << "\n";
}


Human::Human(std::unique_ptr<IFood> food) : _food(std::move(food)) {
}
