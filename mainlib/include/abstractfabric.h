...
class JalousieActor {
 public:
  ...
  float getActualPosition() const { return m_actualPosition; }
  void up(float value) {
...
class HomebusJalousieActor : public JalousieActor {
 public:
  float readActualPosition() override { return 0.0; }
};
...
class HomeController {
 public:
  explicit HomeController(HomeAutomationFabric* fabric) : m_fabric{fabric} {
    createProducts();
  }
  ...
  void up(float value) const { m_jalousieActor->up(value); }

  void down(float value) const { m_jalousieActor->down(value); }
